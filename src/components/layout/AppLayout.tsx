...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  if (!page && path.startsWith("/tasks/")) page = { title: "Task Sheet", subtitle: "Board view with workflow automation" };
  if (!page && path.startsWith("/workflow/")) page = { title: "Workflow Builder", subtitle: "Drag & drop steps to build your automation" };
    ...
    <SidebarProvider>
      <div className="min-h-screen flex w-full mesh-bg dot-grid">
        ...
        <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
          {/* Top bar */}
          <header className="h-14 flex items-center justify-between border-b border-white/5 glass-strong px-5 sticky top-0 z-30 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button className="w-8 h-8 rounded-lg glass glass-hover flex items-center justify-center text-muted-foreground hover:text-foreground">
                <Search className="w-4 h-4" />
              </button>
              <button className="w-8 h-8 rounded-lg glass glass-hover flex items-center justify-center text-muted-foreground hover:text-foreground relative">
